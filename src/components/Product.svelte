...
<SectionWrapper id="product">
  <div class="flex flex-col gap-10 sm:gap-14 md:gap-24 flex-1 
  items-center justify-center pb-10 md:pb-14">
    <div class="flex flex-col gap-2">
      ...
      <h3 class="text-4xl sm:text-5xl md:text-6xl 
      max-w-[1000px] mx-auto width-full font-semibold text-center">
        Features For everyone</h3>

      {#each productFeatures as productFeature, index}
        <ProductCard {productFeature} {index}>
          {#if index === 0}
            Experience a beginner exercise 
            <span class="text-indigo-400 font-medium">master</span> class
          {:else if index === 1}
